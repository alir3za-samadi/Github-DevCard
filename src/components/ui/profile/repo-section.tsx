...
import type { GithubRepos, SortOptionValue } from "@/lib/types";

const ITEMS_PER_PAGE = 9;

export default function RepoSection({ repos }: { repos: GithubRepos }) {
  ...
  function handleValueChange(value: SortOptionValue) {
    setSortBy(value);
    setCurrentPage(1);
  }
...
function Sort({
  ...
  onValueChange: (value: SortOptionValue) => void;
}) {
  return (
    <div className="flex items-center w-1/2 gap-2 text-xs text-muted-foreground">
      ...
      <Select
        items={SORT_OPTIONS}
        value={sortBy}
        onValueChange={(val) => onValueChange(val as SortOptionValue)}
      >
